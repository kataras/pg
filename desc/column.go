...
type (
	...
	Column struct {
		...
		AutoGenerated bool
		// GeneratedExpression is the SQL expression for a GENERATED ALWAYS AS (...) STORED column.
		// It sets AutoGenerated to true automatically.
		// When non-empty, the column is created as a stored generated column and excluded from INSERT/UPDATE.
		// E.g. pg:"type=int,generated=EXTRACT(EPOCH FROM prepare_duration)::int / 60"
...
func (c *Column) IsGenerated() bool {
	return c.IsGeneratedPrimaryUUID() || c.IsGeneratedTimestamp() || c.GeneratedExpression != ""
}
...
func (c *Column) FieldTagString(strict bool) string {
	...
	if strict {
		writeTagProp(b, ",generated=%s", c.GeneratedExpression)
	} else if c.GeneratedExpression != "" {
		writeTagProp(b, ",generated", true)
	}
	if strict {
